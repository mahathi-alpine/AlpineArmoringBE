  ...
  async beforeUpdate(event) {
    ...
    try {
      ...
      if (event.params.data.localizations?.length > 0) {        
        ...
        const originalEntry = await strapi.documents('api::blog-evergreen.blog-evergreen').findOne({
          documentId: "__TODO__"
        });
      }
    } catch (error) {
    ...
    if (event.params.data.publishedAt && !event.params.data.title) {
      const fullEntity = await strapi.documents('api::blog-evergreen.blog-evergreen').findOne({
        documentId: "__TODO__",
        populate: '*'
      });
...
const getLocale = async (id) => {
  const res = await strapi.service("api::blog-evergreen.blog-evergreen").findOne(id);
